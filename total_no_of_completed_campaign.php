        ...
        <div class="warper container-fluid">
             ...
             <div class="col-md-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">Total No Of Completed Campaign</div>
...
<br><table class="table table-bordered order-table table">
								...
								AND c.active_or_inactive='2'  limit $start,$limit
								...
								AND b.camp_active_status='1' AND b.camp_end_date <= '$myDate'
								AND c.active_or_inactive='4' limit $start,$limit ";					
						
							$no=$start;
								$query=mysqli_query($GLOBALS["___mysqli_ston"], $sql) or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
					...
					?>
                                <tbody>
                                  <tr>
								    <input type="hidden" id="camp_id" value="<?php  echo $row['camp_id'];?>">
                                    <td><?php  echo $no;?></td>
									...
									<?php 
									if($active_or_inactive=='2')
									{
									?>
									<td>
									 <select name="vin_id" id="vin_id" required>
											<option value="">Select name</option>
												<?php
												$sql="select * from  cust_vinyler_info";
												$query=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
												while($row1=mysqli_fetch_array($query))
												{
												
												?>
												 <option value="<?php echo $row1['vn_id']; ?>"><?php echo $row1['vn_name'];?></option>
												<?php } ?>
										</select>
									...
									else if($active_or_inactive=='4')
									{
									?>
									<td>Campaign has unwrapped</td>
									<?php
									}
					...
					<?php 
									...
									{
									?>
								<td><input type="button"  id="active" value="Send TO Unwrapping"   class="btn btn-success" onclick="send_to_vinyl();"></td>
									<?php
									}
									else if($active_or_inactive=='4')
									{
           ...
           <a href="total_no_of_completed_campaign_download.php" class="btn btn-purple btn-md">Download</a>
	...
	{	
			...
			var vin_id = document.getElementById("vin_id").value;
			
			
			
	 var url="update_send_vinyl.php?camp_id="+camp_id+"&vin_id="+vin_id;
	
 //xmlhttp.open("GET","load_search.php?tyre_id="+tyre_id+"&brand_id="+brand_id+"&vehicle_id="+vehicle_id+"&position_id="+position_id+"&tyrebrand_id="+tyrebrand_id,true);
